...
function displaySheet(_sheet) {
    ...
    for (let i = 0; i < cards.length; i++) {
        for (let k = 0; k < cards[i].length; k++) {
            ...
            let cardDomText = document.createElement("p");
...
function updateSheet(_sheet) {
    ...
    for (let i = 0; i < cards.length; i++) {
        for (let k = 0; k < cards[i].length; k++) {
            ...
            if (curCard.getAttribute("status") == "selected") {
                let xEl = document.createElement("div");
                xEl.style.backgroundImage = "./assets/x" + Math.round(Math.random() * 3) + ".png";
                curCard.append(xEl);
            }
        }
    }
    if (_sheet.checkForBingo()) {
        let bingoEl = document.createElement("p");
...
async function getTextsFromDb(_amount = 25) {
    try {
        let cardTexts = [];
        ...
        let chosenCardTexts = getRandomItemsFromArray(cardTexts, _amount);
...
function submitCardToDb() {
    const feedbackEl = getId("feedback");
    feedbackEl.innerHTML = "";
    try {
        const newCardEl = getId("cardTextInput");
...
function generateSheet(_texts) {
    ...
    for (let i = 0; i <= _texts.length; i++) {
        ...
        let card = new Card(_texts[i]);
        curRow.push(card);
    }
    console.log(cards);
    let sheet = new Sheet(cards, cards.length);
    return sheet;
}
